...
\newpage
    ...
    \subsubsection{Kontrast \& Gamma \\}

    Kontrast und Gamma werte sind typische Stellräder für die Bearbeitung von Bilddaten, sie sind wie Folgt mathematisch definiert:\\
        \begin{equation} \label{eq:1}
            I_{out} = {I_{in}}^{\gamma}
        \end{equation}
        
        Wobei:
        $ (I_{out}, I_{in}) \in {\Bbb R} \| \{0,1\} $\\
        \newline
        Für die Verwendung innerhalb von Computergrafiken müssen dieser werte auf unit8 konvertiert werden und enthalten so werte zwischen 0-255.\\
        $ \gamma $-werte $ > 1 $ werden das Bild insgesamt heller erscheinen lassen, während werte $ < 1 $ das Bild dunkler darstellen.\\



    \subsubsection{Dark Channel prior  \& Histogram matching\\}
        ''Histogram matching'' beschreibt eine weiter Möglichkeit der Bildbearbeitung die auf die auf Kontrast und Gamma Korrektur aufbaut, während Kontrast und gamma-Korrektur nur jeweils einen teil der Histogram kurve bearbeiten wird beim Histogram matching das Histogram des Bildes an eine beliebige Funktion angepasst. \\
