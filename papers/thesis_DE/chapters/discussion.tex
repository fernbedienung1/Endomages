...
\end{itemize}
    ...
    \paragraph{Rauch- Arten}
        ...
        Die Erzeugten Daten enthalten nur Rauch der von einen Bipolaren Elektokauter erzeugt wurde bzw Dampf der mittels eines Wasserkochers eingespeist wurde und sind somit nicht für alle arten von Rauch gültig.
...
\subsection{Erkennung durch Neuronale Netze}
    ...
    was auch den ursprünglichen Startpunkt der Thesis darstellen sollte.\\
    Die Erkennung von mustern durch neuronale Netze hat in den letzten Jahren einen erheblichen Boom erfahren und erzielt mit unter sehr gute Ergebnisse in der Bearbeitung von Videos. \\
    Paper wie \cite{NN_Style} zeigen eine Möglichkeit nicht nur bestimmte Muster in Bildern zu erkennen, sondern auch diese Muster zu verändern und an andere Anzupassen. Beispielhaft wird dies mit den Bildern verdeutlicht die ein Foto an den Stil berühmter Maler anpassen \ref{fig:paint}. \\
        \begin{figure}[H]
            \includegraphics[width=\textwidth]{pics/usable_unsure/nn_style.jpg}
            \caption{Eingangs Foto links wird zum Gemälde Transformiert \cite{NN_Style}}
            \label{fig:paint}
        \end{figure}

        Der Ursprüngliche Plan dieser Thesis war es hier anzuknüpfen und auf den Basis die Konvertierungen vorzunehmen.\\
        Die Verwendung von Python in Kombination mit den Bibliotheken OpenCV für die Bildbearbeitung sowie Tensor-Flow \cite{tf} sind hier ein hervorragender und erweiterbarer Startpunkt. Da beide Bibliotheken via Python leicht zugänglich sind und für tiefgehende Performance Optimierungen eine C++ Schnittstelle bereitstellen.
