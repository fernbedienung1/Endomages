    ...
    \subsection{Histogramme als Vergleichsgrundlage}
        ...
        \newpage
         \subsection{Anwendung von Bildanpassungsalgorithmen}
            ...
            Die unter \ref{sec:Fundamentals} beschriebenen Algorithmen wirken sehen im code folgendermassen aus \ref{code:examples}.
            ...
            Die Eingabedatei \ref{fig:inputData} wird als Input verwendet für die Anpassung. Um das aufgehellte Bild \ref{fig:brightData} zu erhalten wird die mathematische Funktion \\

            \begin{equation} \label{eq:bright}
                g(x) = x + x*0.1
            \end{equation}

            Anzuwenden. Zeile 6 des Codes \ref{code:fastmatch} Zeigt diese Funktion als matlab bzw Octave code.\\
            Selbes verfahren wird mit den anderen Funktionen \ref{eq:gamma} sowie der Formel\ref{eq:sigmoid}.
            ...
            auch diese beiden Formel finden sich im Code \ref{code:fastmatch} wieder der die Bilder \ref{fig:imageAdjust} generiert.
            Formel \ref{eq:gamma} wird in Zeile 7 als Code beschrieben und Formel \ref{eq:sigmoid} darauf folgend in Zeile 8.

            \lstinputlisting[language=matlab, caption={Code es Histogrammatching}, label={code:fastmatch}, firstline=1, lastline=13]
                {../../work/mathe/own/examples.m}
            
            Mit dieser anfänglichen aber flexiblen Methode ist es möglich das Bild sichtlich zu verändern und auch in einem gewissen Ramen Verbesserungen zu erzeugen. Für ein ''Proof of Concept'' genügt diese Implementierung. Sie ermöglicht es in den Folgeschritten einer Verschmutzungserkennung verschiedene Algorithmen auf verschiedene teile des Bildes anzuwenden. \\
            Dazu können verschiedene Funktion ( wie \ref{eq:bright}, \ref{eq:gamma}, \ref{eq:sigmoid} ) definiert werden und auf einzelteile des Bildes angewandt werden.\\
