...
%%%%%%%%%%%%%%%%%%%%%%% Pictures of Test Setup %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    \paragraph{Fazit des ersten Prototpen}
        Die Generierung von Daten ist mit ''Haushaltsmittlen'' nicht zu ermöglichen, zwar ist es möglich Rauch zu erzeugen und die ersten Tests und Versuche decken einige Fallstricke auf. Jedoch nicht in einem Realen oder brauchbaren Ausmaß. Für die brauchbare Verwendung für die Algorithmen muss mit Professionelleren Werkzeugen gearbeitet werden. Die Verbesserungen sind insbesondere in den Bereichen: \\
        \begin{itemize}
        ...
        gemacht werden.\\
        Folglich ist es notwendig einen 2ten Laborversuch durchzuführen welcher die obrigen Punkte berücksichtigt.
    ...
    Die Verbesserungen in \ref{tab:improvements} erzeugen Daten die für eine weiterverarbeitung in frage kommen, jedoch noch qualitativ mit mit echten Daten gegenübergestellt werden müssen. \\
    
    Die generierten bilder sollten im laufe der Arbeit noch Chirurgen zur validierung vorgelegt werden, aufgrund schlechter kontakte und verfügbarkeiten der Chriurgen ist dies Jedoch zeitlich nicht mehr im Rahmen der Arbeit zustande gekommen. \\
    Die gerneriten Daten bedürfen daher noch einer Prüfung durch Mediziner, Chirurgen oder andwereitig Qualifizierte Experten, die bestätigen oder verneinen das es sich hierbei um realitätsnahe Daten handelt.\\
    Ausserdem ist zu klären unter welchen Aspekten sich die generierten Daten letztendlich von Realen unterscheiden und in wiefern dies eine Weiterverarbeitung beeinflusst. 
...
\subsubsection{Reale Daten}
    ...
    \begin{figure}[h]
        ...
        \label{fig:compareP1P2}
    \end{figure}


\newpage
\section{Daten Bearbeitung}
    Die Bearbeitung der generierten Daten teil sich in mehrere Teile.
    ...
    Im Anschluss werden die Bilder mit Hilfe von Histogrammen verglichen und bearbeitet. \\
    Eine Implementation von Rauch oder Dampf Erkennung ist nicht mehr teil der Arbeit.

    \subsection{Tools und Sprachen}
        Die Verarbeitung der Daten erfolgt auf einem Dell Latitude \cite{laptop} als Host-System, in Verbindung mit einer Ubuntu18.04 Virtuellen Maschine die mittels VMware-Player auf dem Hostsystem die Verwendung der Bash und anderen Programierwerkzeugen ermöglicht. \\
        ...
        installiert werden können. Die packte sind: \\
        ...
        Insbesondere die openCV-python bibliothek wurde verwendet, da es sich hierbei um eine C++-Bibliothek mit einem Python-wrapper handelt, die Bildbearbeitung in Bildern sowie Videos zulässt. In Verbindung mit Numpy und matplotlib ist es so möglich schnell zu prototypischen Ergebnissen zu gelangen.\\
        Python als geskriptete Sprache erlaubt eine schnelle Implementation des Codes, jedoch bleiben die Ausführungszeiten hinter derer von kompilierten sprachen wie C++ zurück. Da die openCV-bibliothek für python nur ein wrapper um die eigentliche C++ Implementierung ist, kann so erwartet werden das sich etwaiger Portierungsaufwand gering hält. Desweiteren ist openCV eine in sowohl in Wissenschaft also auch in der Industrie weit verbreitete Bibliothek für Bildbearbeitungen.\\
        Matplotlib und numpy sind Mathematische bibliothek die bei der erstellung von Beispielsweise Histogrammen vorgefertige algorithmen mitbringen auf die zugegriffen werden kann. \\ 
        ...
        Ein weiterer vorteil von Octave ist die produzierte Code, der sehr mathematisch zu lesen ist und so eine einfache übersetzung von Formeln in Quelltext ermöglicht.\\
